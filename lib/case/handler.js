...
function classnamesIdentifierChecker(node) {
    const globalClassNamesIdentifier = config$.getState('globalClassNamesIdentifier');
    ...
    const properties = node.properties.map((property) => {
        ...
        if (propertyKeyNodeType === 'Identifier' || isLiteralType(propertyKeyNodeType)) {
            const keyString = isLiteralType(propertyKeyNodeType)
                ? property.key.value
                : property.key.name;
            const { computed } = config$.getConfig;
            const specifier = config$.getState('specifier');
...
function conditionalSideHandler(node) {
    ...
    if (node.type === 'Identifier' && node.name === 'undefined') {
        return { transfered: true, node: j.stringLiteral('') };
    }
