    ...
    j(path).replaceWith(p => {
        ...
        const arguments = origins.map((node) => {
            if (node.type === 'StringLiteral') {
                return j.jsxExpressionContainer(j.memberExpression(j.identifier(specifier), j.stringLiteral(node.value), true));
            }
            if (node.type === 'ObjectExpression') {
                const properties = node.properties.map((property) => {
                    ...
                    return j.objectProperty(key, property.value)
                })
...
async function executor(entry = defaultEntry) {
    ...
    console.info("entries:", entry, "file count:", entries.length)
    entries.forEach(async (entry) => {
        try {
            const originData = getAstFromSource(entry);

            const { specifier, isImportLess } = importDeclarationTransfer(originData);

            if (!isImportLess) {
                return
            }
            console.info('current file:', entry)
            ...
            console.log("This file is formatted:", entry)

            fs.writeFileSync(entry, formatted);

            console.info("This file is done:", entry)

        } catch (error) {
            console.error("This file is error:", entry)
            console.error(error)
        }
    });
}
